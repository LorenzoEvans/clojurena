...
(defn toggle-block-status [channel-name id status auth & user pass]
 "Changes the status of a block, specifically it's connection to a channel."
  (async
    (client/put (str base-url channel-name "/" "blocks/" id "/" selection) {:oauth-token (if (not= nil auth) auth) 
                                                                    :status status
